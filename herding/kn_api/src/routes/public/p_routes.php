...
select (st_dump(geom)).geom from tbl_area_a_and_b_combined where class = 'Area B'
 ...
 FROM (SELECT 'Feature' As type
   ...
   FROM tbl_area_a_and_b_combined As lg   ) As f )  As fc;";
	...
	//Declared State Land
    $app->get(getenv("ROOT_PATH") . '/tbl_declared_state_land', function (Request $request, Response $response, $args) use($app) {
        $sql = "SELECT row_to_json(fc)
                 FROM ( SELECT 'FeatureCollection' As type, array_to_json(array_agg(f)) As features
                 FROM (SELECT 'Feature' As type
                   ...
                   FROM tbl_declared_state_land As lg   ) As f )  As fc;";
        $result = DbHelper::getDataArray($sql, []);
        ...
        return $response->withHeader('Content-Type', 'application/json')
            ...
            ->withHeader('Access-Control-Allow-Headers', 'X-Requested-With, Content-Type, Accept, Origin, Authorization')
            ->withHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, PATCH, OPTIONS');;
    })->setName('tbl_declared_state_land');
//tabu
    $app->get(getenv("ROOT_PATH") . '/tbl_tabu_state_land', function (Request $request, Response $response, $args) use($app) {
        $sql = "SELECT row_to_json(fc)
                 FROM ( SELECT 'FeatureCollection' As type, array_to_json(array_agg(f)) As features
                 FROM (SELECT 'Feature' As type
                    , ST_AsGeoJSON(St_transform(lg.geom,4326))::json As geometry
                   ...
                   FROM tbl_tabu_state_land As lg   ) As f )  As fc;";
        $result = DbHelper::getDataArray($sql, []);
...
//                   FROM tbl_herding_farms As lg  where type is not null  ) As f )  As fc;";
    ...
    $app->get(getenv("ROOT_PATH") . '/herding_outpost_post_2022', function (Request $request, Response $response, $args) use($app) {
        ...
        //                   FROM tbl_herding_farms As lg  where type is not null  ) As f )  As fc;";
                ...
                $sql = "SELECT row_to_json(fc)
                         ...
                         FROM (SELECT 'Feature' As type
                            , ST_AsGeoJSON(St_transform(lg.geom,4326))::json As geometry
                            , row_to_json((SELECT l FROM (SELECT gid, id, name, קהילו as titt_heb, year as year_established, ערבית as titt_arb, name_engli as titt_eng) As l
                              )) As properties
	...
	$app->get(getenv("ROOT_PATH") . '/babachy_israeli', function (Request $request, Response $response, $args) use($app) {
        $sql = "SELECT row_to_json(fc)
                 ...
                 FROM (SELECT 'Feature' As type
                   ...
                   FROM tbl_babach_yisraeli_nov As lg   ) As f )  As fc;";
        $result = DbHelper::getDataArray($sql, []);
	...
	//tbl_firing_zone
    $app->get(getenv("ROOT_PATH") . '/tbl_firing_zone', function (Request $request, Response $response, $args) use($app) {
        $sql = "SELECT row_to_json(fc)
                 FROM ( SELECT 'FeatureCollection' As type, array_to_json(array_agg(f)) As features
                 FROM (SELECT 'Feature' As type
                    , ST_AsGeoJSON(St_transform(lg.geom,4326))::json As geometry
                   ...
                   FROM tbl_firing_zone As lg   ) As f )  As fc;";
        $result = DbHelper::getDataArray($sql, []);
	...
	//Nature Reserve
    $app->get(getenv("ROOT_PATH") . '/tbl_nature_reserve', function (Request $request, Response $response, $args) use($app) {
        $sql = "SELECT row_to_json(fc)
                 FROM ( SELECT 'FeatureCollection' As type, array_to_json(array_agg(f)) As features
                 FROM (SELECT 'Feature' As type
                    , ST_AsGeoJSON(St_transform(lg.geom,4326))::json As geometry
