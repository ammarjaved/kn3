   ...
   FROM tbl_area_a_and_b_combined As lg   ) As f )  As fc;";
    ...
    $app->get(getenv("ROOT_PATH") . '/area_b_violations', function (Request $request, Response $response, $args) use($app) {
        $sql = "SELECT row_to_json(fc)
                 ...
                 FROM (SELECT 'Feature' As type
                    , ST_AsGeoJSON(lg.geom)::json As geometry
                    , row_to_json((SELECT l FROM (SELECT fid, id, photoid, categoryid, cat_heb, set_heb, pal_heb, cat_arb, cat_eng, set_eng, set_arb, pal_arb, pal_eng, art_heb1, art_eng1, art_arb1, art_heb2, art_eng2, art_arb2,art_heb3, art_eng3, art_arb3) As l
                      )) As properties
    ...
    $app->get(getenv("ROOT_PATH") . '/area_b_demolitions', function (Request $request, Response $response, $args) use($app) {
        $sql = "SELECT row_to_json(fc)
                 ...
                 FROM (SELECT 'Feature' As type
                    , ST_AsGeoJSON(lg.geom)::json As geometry
                    , row_to_json((SELECT l FROM (SELECT fid, entity, layer, color, linetype, elevation, linewt, refname, angle) As l
                      )) As properties
                   FROM tbl_area_b_demolitions As lg   ) As f )  As fc;";
        $result = DbHelper::getDataArray($sql, []);
