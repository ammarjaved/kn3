        ...
        $(opacity_slider_div).slider({
          ...
          slide: function ( event, ui ) {
            ...
            for(var i=0; i<opacity_layers.length; i++){
                var lyr = opacity_layers[i];
                try{
                    lyr.setStyle({opacity: slider_value, fillOpacity: slider_value});
                }catch(err){
                    
                }
            }

            // var layers = opacity_layer._layers;
            // for(var layer in layers){
            //     layers[layer].setOpacity(slider_value);    
            // }
          }
        });
